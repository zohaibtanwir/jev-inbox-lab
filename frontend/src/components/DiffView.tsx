...
function RunHead({ label, r }: { label: string; r: DiffPayload["a"] }) {
  return (
    <div className="rounded border border-neutral-800 p-2 text-xs">
      <span className="mr-2 rounded bg-emerald-900 px-1 text-emerald-200">{label}</span>
      <span className="text-neutral-200">run #{r.id}</span>
      <span className="ml-2 text-neutral-500">{new Date(r.started_at).toLocaleString()}</span>
      <span className="ml-2 text-neutral-400">{r.model_version ?? "no model"}</span>
      <span className="ml-2 text-neutral-600">{r.email_count} emails · {r.worker_count} workers</span>
    </div>
  );
}
...
function QuestionSideBySide({ change }: { change: DiffPayload["question_changes"][number] }) {
  ...
  const render = (lines: string[], other: Set<string>) =>
    lines.map((l, i) => (
      <div key={i} className={other.has(l) ? "text-neutral-400" : "rounded bg-amber-950/60 px-1 text-amber-200"}>
        {l || " "}
      </div>
    ));
  return (
    <div className="rounded border border-neutral-800">
      <div className="border-b border-neutral-800 px-2 py-1 text-xs">
        <span className="text-neutral-100">{change.question_id}</span>
        <span className="ml-2 text-neutral-500">{change.field}</span>
      </div>
      <div className="grid grid-cols-2 gap-px bg-neutral-800 text-xs">
        <div className="whitespace-pre-wrap bg-neutral-950 p-2 font-mono">{render(aLines, bSet)}</div>
        <div className="whitespace-pre-wrap bg-neutral-950 p-2 font-mono">{render(bLines, aSet)}</div>
      </div>
    </div>
  );
}
...
export default function DiffView({ a, b, emails, onClose }: Props) {
  ...
  const who = (id: string) => {
    ...
    return e ? (
      <span>
        <span className="text-neutral-300">{displayName(e.from)}</span>
        ...
        <>
          ...
          <div className="text-xs text-neutral-500">
            ...
            {diff.a.model_version !== diff.b.model_version && (
              <span className="ml-2 text-amber-300">model version differs — answer changes may be the model, not your edit</span>
            )}
          </div>

          <section>
            <h3 className="mb-1 text-xs uppercase tracking-wide text-neutral-500">
              Question changes <span className="text-neutral-600">({diff.question_changes.length})</span>
            </h3>
            {diff.question_changes.length === 0 ? (
              <div className="text-neutral-600">Identical question sets.</div>
            ) : (
          ...
          <section>
            <h3 className="mb-1 text-xs uppercase tracking-wide text-neutral-500">
              Category flips <span className="text-neutral-600">({diff.category_flips.length})</span>
            </h3>
                  ...
                  {diff.category_flips.map((f, i) => (
                    <tr key={i} className="border-t border-neutral-900">
                      ...
                      <td className="py-1 font-mono">
                        <span className="text-neutral-400">{f.a}</span>
                        <span className="mx-2 text-neutral-600">→</span>
                        <span className="text-neutral-100">{f.b}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {(["confidence_deltas", "noul_deltas"] as const).map((key) => (
              <section key={key}>
                <h3 className="mb-1 text-xs uppercase tracking-wide text-neutral-500">
                  ...
                  <span className="text-neutral-600">({diff[key].length})</span>
                </h3>
                {diff[key].length === 0 ? (
                  <div className="text-neutral-600">None beyond ±{diff.delta_threshold.toFixed(2)}.</div>
                ) : (
                  <table className="w-full text-xs">
                    <tbody>
                      {[...diff[key]].sort((x, y) => Math.abs(y.delta) - Math.abs(x.delta)).map((d, i) => (
                        <tr key={i} className="border-t border-neutral-900">
                          ...
                          <td className="py-1 font-mono tabular-nums">
                            <span className="text-neutral-400">{d.a.toFixed(2)}</span>
                            <span className="mx-1 text-neutral-600">→</span>
                            <span className="text-neutral-100">{d.b.toFixed(2)}</span>
                            <span className={`ml-2 ${d.delta > 0 ? "text-emerald-300" : "text-red-300"}`}>{sign(d.delta)}</span>
                          </td>
                        </tr>
                      ))}
                    </tbody>
