...
function TextArea({ value, onChange, rows = 3, mono = false }: { value: string; onChange: (v: string) => void; rows?: number; mono?: boolean }) {
  return (
    <textarea
      ...
      className={`w-full resize-y rounded border border-neutral-200 bg-white p-1.5 text-xs text-neutral-800 focus:border-neutral-500 focus:outline-none ${mono ? "font-mono" : ""}`}
    />
  );
}
      ...
      <div className="space-y-1">
        {levels.map((l, i) => (
          <div key={i} className="flex items-start gap-2">
            ...
            <button type="button" className="pt-1 text-xs text-neutral-400 hover:text-red-600" onClick={() => onChange(levels.filter((_, j) => j !== i))} title="Remove level">
              ×
            </button>
          </div>
        ))}
        <button type="button" className="text-xs text-neutral-500 hover:text-neutral-800" onClick={() => onChange([...levels, ""])}>
          + add level (highest)
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-1">
      {entries.map(([k, v], i) => (
        <div key={i} className="flex items-start gap-2">
          {fixedKeys ? (
            <span className="w-24 shrink-0 pt-1.5 font-mono text-xs text-neutral-600">{k}</span>
          ) : (
            <input
              value={k}
              onChange={(e) => set(entries.map((x, j) => (j === i ? [e.target.value, x[1]] : x)))}
              className="w-32 shrink-0 rounded border border-neutral-200 bg-white p-1.5 font-mono text-xs text-neutral-800 focus:border-neutral-500 focus:outline-none"
            />
          )}
          <TextArea value={v} rows={2} onChange={(nv) => set(entries.map((x, j) => (j === i ? [x[0], nv] : x)))} />
          {!fixedKeys && (
            <button type="button" className="pt-1 text-xs text-neutral-400 hover:text-red-600" onClick={() => set(entries.filter((_, j) => j !== i))} title="Remove option">
              ×
            </button>
          )}
        </div>
      ...
      {!fixedKeys && (
        <button type="button" className="text-xs text-neutral-500 hover:text-neutral-800" onClick={() => set([...entries, ["new_option", ""]])}>
          + add option
        </button>
      )}
    </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-xs text-neutral-500">
            current: <span className="text-neutral-700">{initial.version}</span>
            {initial.version !== initial.default_version && <span className="ml-1 text-neutral-400">(edited from {initial.default_version})</span>}
            {" · "}saved edits apply to future runs only; every run keeps the exact set it sent.
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => setRaw(!raw)} className="rounded border border-neutral-300 px-2 py-1 text-xs text-neutral-700 hover:bg-neutral-100">
            {raw ? "Form" : "Raw JSON"}
          </button>
          <button
            type="button"
            ...
            onClick={() => save(null)}
            className="rounded border border-neutral-300 px-2 py-1 text-xs text-neutral-700 hover:bg-neutral-100 disabled:cursor-not-allowed disabled:opacity-40"
          >
          ...
          <button
            ...
            className="rounded bg-emerald-600 px-3 py-1 text-xs font-medium text-white hover:bg-emerald-500 disabled:cursor-not-allowed disabled:bg-neutral-200 disabled:text-neutral-500"
          >
            {saving ? "Saving…" : "Save"}
          </button>
      ...
      {raw ? (
        <div>
          <textarea
            ...
            rows={30}
            spellCheck={false}
            className="w-full rounded border border-neutral-200 bg-white p-2 font-mono text-xs text-neutral-800 focus:border-neutral-500 focus:outline-none"
          />
          {rawError && <div className="mt-1 text-xs text-red-700">{rawError}</div>}
          <div className="mt-1 text-xs text-neutral-400">Add or remove whole questions here. Ids are for your code only — the model never sees them.</div>
        </div>
      ) : (
        <div className="space-y-3">
          {Object.entries(draft).map(([qid, q]) => (
            <div key={qid} className="rounded border border-neutral-200 p-3">
              <div className="mb-2 flex items-center gap-2 text-xs">
                <span className="font-mono text-neutral-900">{qid}</span>
                <span className="rounded bg-neutral-100 px-1 text-neutral-600">{q.type}</span>
                <button
                  type="button"
