...
- Track work in `bd`: file issues for follow-ups, close what is finished.
- Never put email contents, addresses or subjects from the corpus into a bead.
- The git rules above still apply: no commits or pushes unless asked. There
  is no remote. Issues live in a local Dolt DB; `.beads/issues.jsonl` is a
  passive export.
